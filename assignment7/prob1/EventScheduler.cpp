...
void EventScheduler::show_relevant_events(Date theDay,Date theDayend) {
    for (auto ev: events) {
        Date Daytemp=theDay;
        vector<int>duplication;
        int count;
        while(theDay<=theDayend) {
            if (ev->relevant_to(theDay)) {
                count=0;
                for(int i=0;i<duplication.size();i++){
                    if ((ev->get_id())==duplication[i]) {
                        count++;
                        break;
                    }
                }
                if (count==0){
                    duplication.push_back(ev->get_id());
                    ev->print(cout);
                    cout << endl;
                }
            }
            theDay.Dateadd(theDay);
        }
        theDay=Daytemp;
    }
}
...
void EventScheduler::save_event(string name){
    ofstream fout;
    fout.open(name);
    for (auto ev: events) {
        ev->print(fout);
